...
export async function generateMetadata(): Promise<Metadata> {
  const city = await getCityBySlug("plzen");
  return {
    title: city?.seoTitle,
...
export default async function BarberKurzPlzenPage() {
  const city = (await getCityBySlug("plzen"))!;
  const cityLocations = await getLocationsByIds(city.locationIds);
  const courses = await getAllCourses();
  const allTestimonials = await getAllTestimonials();
  const cityTestimonials = allTestimonials.filter((t) => t.city === "plzen" || !t.city);
