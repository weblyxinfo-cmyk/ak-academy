...
import { getCityBySlug, getLocationsByIds, getAllCourses, getAllTestimonials, getComparisonRows, getGeneralFaq } from "@/db/queries";
...
export default async function BarberKurzBerounPage() {
  const city = (await getCityBySlug("beroun"))!;
  const cityLocations = await getLocationsByIds(city.locationIds);
  const courses = await getAllCourses();
  const allTestimonials = await getAllTestimonials();
  const cityTestimonials = allTestimonials.filter((t) => t.city === "beroun" || !t.city);
