...
export async function generateMetadata(): Promise<Metadata> {
  const city = await getCityBySlug("praha");
  return {
    title: city?.seoTitle,
...
export default async function BarberKurzPrahaPage() {
  const city = (await getCityBySlug("praha"))!;
  const cityLocations = await getLocationsByIds(city.locationIds);
  const courses = await getAllCourses();
  const allTestimonials = await getAllTestimonials();
  const cityTestimonials = allTestimonials.filter((t) => t.city === "praha" || !t.city);
