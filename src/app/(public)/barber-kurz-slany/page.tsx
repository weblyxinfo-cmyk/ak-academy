...
export async function generateMetadata(): Promise<Metadata> {
  const city = await getCityBySlug("slany");
  return {
    title: city?.seoTitle,
...
export default async function BarberKurzSlanyPage() {
  const city = (await getCityBySlug("slany"))!;
  const cityLocations = await getLocationsByIds(city.locationIds);
  const courses = await getAllCourses();
  const allTestimonials = await getAllTestimonials();
  const cityTestimonials = allTestimonials.filter((t) => t.city === "slany" || !t.city);
